...
public final class KitsHabilitadosGUI implements Listener {
	...
	private void onInventoryClick(InventoryClickEvent event) {
		...
		if (event.getWhoClicked() instanceof Player && event.getInventory().getName().equals("Kits Habilitados")
			...
			if (display.equals("§fVisualizando: §aKits Habilitados")) {
			...
			if (display.startsWith("§aKit ")) {
				Kit kit = system.getKitByName(display.replace("§aKit ", ""));
				if (kit != null) {
					if (event.isLeftClick()) {
	...
	public static void openGUI(Player player) {
		...
		inv.setItem(4,
				new Stack(Material.STAINED_GLASS_PANE, 1, 5).display("§fVisualizando: §aKits Habilitados").lore(" ",
						" §8» §6Todos os Kits", " §8» §eSeus Kits", " §2» §aKits Habilitados",
		...
		system.getKits().stream().filter(kit -> kit.isEnable()).forEach(kit -> {
			Stack icon = kit.getIcon().clone().hideAttributes().display("§aKit " + kit.getName())
