...
public final class WarpsDesabilitadasGUI implements Listener {
	...
	private void onInventoryClick(InventoryClickEvent event) {
		...
		if (system == null || !(system instanceof WarpsSystem) || !system.isEnable())
			return;
		if (event.getWhoClicked() instanceof Player && event.getInventory().getName().equals("Warps Desabilitadas")
			...
			if (display.equals("§fVisualizando: §cWarps Desabilitadas")) {
			...
			if (display.startsWith("§cWarp ")) {
				Warp warp = system.getWarpByName(display.replace("§cWarp ", ""));
	...
	public static void openGUI(Player player) {
		...
		Inventory inv = Bukkit.createInventory(null, 27, "Warps Desabilitadas");
		...
		inv.setItem(4,
				new Stack(Material.STAINED_GLASS_PANE, 1, 14).display("§fVisualizando: §cWarps Desabilitadas").lore(" ",
						" §8» §bWarps", " §8» §aWarps Habilitadas", " §2» §cWarps Desabilitadas", " ",
		...
		warps.forEach(warp -> inv.addItem(warp.getIcon().clone().hideAttributes().hidePotionEffects().hideEnchants()
				.display("§cWarp " + warp.getName()).lore("§7" + warp.getDescription(), 5, 30)));
